...
class MimesisGenerator(MimesisGeneratorBase):
    ...
    def fit(self, default=None) -> float:
...
class MimesisDateTimeGenerator(MimesisGeneratorBase):
    ...
    def make_singleton(
        ...
        """
        Makes a list of just one MimesisDateTimeGenerator.
        """
...
class MimesisTimeGeneratorFactory(GeneratorFactory):
    ...
    def get_generators(self, columns: list[Column], _engine: Engine) -> list[Generator]:
        if len(columns) != 1:
            return []
        column = columns[0]
        ct = get_column_type(column)
...
class ZipfChoiceGenerator(ChoiceGenerator):
    ...
    def generate_data(self, count):
        return [
            dist_gen.zipf_choice(self.values) for _ in range(count)
        ]
