...
class DbConnection:
    def __init__(self, engine):
        """Initialize a potential connction to an Engine or AsyncEngine."""
        self._engine = engine
...
def fix_type(value):
    """
    Cast a value that is problematic in YAML files to a standard type.
    """
...
def fix_types(dics):
    """
    Cast types that are problematic in YAML files to standard types.
    """
...
async def make_src_stats_connection(
    ...
    """
    Make the ``src_stats.yaml`` file
    :param db_conn: The connection to the database to read the stats from.
    """
