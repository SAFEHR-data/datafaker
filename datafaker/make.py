...
def _is_db_generated_pk(column: Column) -> bool:
...
def _get_generator_for_table(
    ...
    if column_choices:
        nonnull_columns = {
            ...
            if not table.columns[col.name].nullable and not _is_db_generated_pk(col)
        }
    else:
        nonnull_columns = {
            str(col.name)
            for col in table.columns
            if not _is_db_generated_pk(col)
        }
