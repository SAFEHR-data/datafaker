...
def _is_db_generated_pk(column: Column, dialect_name: str = "") -> bool:
    """Return True if this column's value should be generated by the database.

    Single-column integer PKs with no FK are left to the DB (IDENTITY / SERIAL)
    on dialects that support it (mssql, postgresql). DuckDB does not auto-generate
    INTEGER PKs so datafaker must supply explicit values there.
    Composite PKs and PK-FK columns must still be generated explicitly.
    """
    if dialect_name not in ("mssql", "postgresql"):
        return False
    return (
        column.primary_key
...
def _get_default_generator(column: Column, dialect_name: str = "") -> RowGeneratorInfo | None:
    """Get default generator information, for the given column.

    Returns None for single-column integer PKs; the database generates those.
    """
    # Single-column integer PKs are generated by the DB (IDENTITY / SERIAL).
    if _is_db_generated_pk(column, dialect_name):
        return None
...
def _get_generator_for_table(
    ...
    if column_choices:
        nonnull_columns = {
            ...
            if not table.columns[col.name].nullable and not _is_db_generated_pk(col, dialect_name)
        }
    else:
        nonnull_columns = {
            str(col.name)
            for col in table.columns
            if not _is_db_generated_pk(col, dialect_name)
        }
    ...
    for column in table.columns:
        if column.name not in columns_covered:
            gen = _get_default_generator(column, dialect_name)
            if gen is not None:
