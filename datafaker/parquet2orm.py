...
def get_parquet_orm(directory: Path) -> dict[str, Any] | None:
    ...
    return {
        name: _get_table_orm(parquet, name, _get_column_words(parquets))
...
def _get_column_words(  # type: ignore[no-any-unimported]
    tables: dict[str, ParquetFile]
) -> list[tuple[str, str, set[str]]]:
    """Get a list of tuples (table_name, column_name, words_in_table_and_column_names)."""
    return [
        (name, column, _get_words(f"{name} {column}"))
        for name, table in tables.items()
        for column in table.columns
    ]


def _get_likely_foreign_key_target(
    column_words: list[tuple[str, str, set[str]]],
    column_name: str,
) -> tuple[str, str] | None:
    """
    Get a foreign key target string if the name suggests it might be.

    :param column_words: A list of all tuples: (table name, column name,
        set of all the words in table name and column name)
    ...
    """
    our_words = set(_word_split_re.split(column_name))
    max_overlap_so_far = len(our_words) - 1
    best_so_far: tuple[str, str] | None = None
    for tn, cn, cws in column_words:
        goodness = len(our_words.intersection(cws))
        if max_overlap_so_far < goodness:
            max_overlap_so_far = goodness
            best_so_far = (tn, cn)
    return best_so_far


def _get_table_orm(  # type: ignore[no-any-unimported]
    table: ParquetFile, name: str, column_words: list[tuple[str, str, set[str]]]
) -> dict[str, Any]:
    """
    ...
    for column, ctype in column_types.items():
        ...
        if ctype == "INTEGER" and words <= name_words:
            likely_primaries.append(column)
            col_orm["primary"] = True
        else:
            col_orm["primary"] = False
        ...
        fk_pair = _get_likely_foreign_key_target(column_words, column)
